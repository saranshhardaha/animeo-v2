        ...
        {schedule &&
          schedule.results.map((anime) => (
            <a
              href={`/anime/${anime.id}`}
              key={anime.id}
              className="group hover:bg-white/10 cursor-pointer flex items-center justify-between gap-2 rounded-md w-full bg-white/5"
            >
              ...
              <div className="flex items-center justify-start px-4 gap-2 bg-white/5 rounded-md p-2 mr-2 opacity-60 group-hover:opacity-100 w-full text-xs max-w-[8rem]">
                ...
                <p>
                  Episode <span>{anime.episode as string}</span>
                </p>
              </div>
            </a>
          ))}
